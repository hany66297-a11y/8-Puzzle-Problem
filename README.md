...
Guarantees the shortest solution but requires high memory.

2️⃣ Depth-First Search (DFS)

Explores one path deeply before backtracking.

Low memory usage but may not find the shortest path.

3️⃣ Iterative Deepening Search (IDS)

Applies DFS repeatedly with increasing depth limits.
...
Always moves toward the neighbor closest to the goal (heuristic).

Very fast but may get stuck in local optima.
